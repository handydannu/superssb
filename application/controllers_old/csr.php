...
class Csr extends CI_Controller {
 ...
 //        // --------- CONTENT ----------------------
	// 	$detail = $this->contents_model->get_single_content(27);

	// 	$c['detail'] = $detail;
	// 	$c['share']  = shareSocmedia($detail['c_title'], current_url());

	// 	$mainData['meta']['descriptions'] = $detail['c_summary'];
	// 	$mainData['meta']['keywords']     = isset($detail['c_keyword'])? $detail['c_keyword'] : str_replace(' ', ',', $detail['c_title']);

	// 	$meta_fb = array(
 //            'url'         => (!empty($_SERVER['HTTPS'])) ? "https://".$_SERVER['SERVER_NAME'].$_SERVER['REQUEST_URI'] : "http://".$_SERVER['SERVER_NAME'].$_SERVER['REQUEST_URI'],
 //            'type'        => 'article',
 //            'title'       => $detail['c_title'],
