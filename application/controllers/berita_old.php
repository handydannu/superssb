...
class Berita extends CI_Controller {
	...
	public function index()
	{
		...
		$mainData['page_title']           = 'Berita BankSulselbar';
		...
		$c['video'] = $video_list;
		$c['foto']	= $this->photo_model->get_single_album(5);
		...
		$mainData['main_content'] = $this->load->view('content/vberita', $c, TRUE);
	...
	public function detail()
	{
		...
		/*$mainData['top_css'] .= add_css('css/captionjs.min.css');
		$mainData['top_css'] .= add_css('css/style.css');
		$mainData['top_css'] .= add_css('css/responsive.css');

		$mainData['bottom_js'] .= add_js('js/jquery-1.9.1.min.js');
		$mainData['bottom_js'] .= add_js('js/bootstrap.min.js');
		
		$mainData['bottom_js'] .= add_js('js/bootstrap-carousel-page-merger.js');
		$mainData['bottom_js'] .= add_js('js/jquery.caption.min.js');
		...
		$video_list = $this->contents_model->get_video_breaking($idkanal, 0, 1);
		$c['video'] = $video_list;
		$c['foto']	= $this->photo_model->get_single_album(5);
