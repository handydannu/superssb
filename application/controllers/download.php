...
class Download extends CI_Controller {
	...
	public function psp()
	{
		...
		$output['page_title'] = 'Download Berkas Pengumuman Sekretaris Perusahaan - Bank Sulselbar';
		$mainData['page_title'] = 'Download Berkas Pengumuman Sekretaris Perusahaan - Bank Sulselbar';
        ...
        if(empty($tahun)){
        	$year = date('Y');
        	$tot = $this->download_model->get_total_all_psp($year);
        	if($tot['itotal']=='0'){										// mengecek jika tahun saat ini tidak ada pada database
        		$show = $this->download_model->get_year_download_sort($id);
        		if($show->num_rows()>0){
        			foreach ($show->result() as $rows) {
		...
		$post 		= $this->download_model->get_all_psp($year, $offset, $limit);
		$total 		= $this->download_model->get_total_all_psp($year);
		...
		// --------- PAGINATION --------------
		$this->load->library('pagination');
		$config['base_url']     = base_url('download/psp/'.$year);
        $config['uri_segment']  = 4;
        $config['total_rows']   = $total['itotal'];
        $config['per_page']     = $limit;
        $config['num_links']    = 3;                // jml links disamping kanan-kiri link yg sdg aktif
		...
		$mainData['navigation']   = $this->load->view('template/vnavigation', NULL, TRUE);
		$mainData['main_content'] = $this->load->view('content/vdownload_psp', $output, TRUE);
		$mainData['footer']       = $this->load->view('template/vfooter', NULL, TRUE);
	...
	public function other()
	{
		...
		$output['page_title'] = ' Download Berkas Pengumuman Lain - Bank Sulselbar';
		$mainData['page_title'] = 'Download Berkas Pengumuman Lain - Bank Sulselbar';
