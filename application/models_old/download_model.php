...
class Download_model extends CI_Model
{
	...
	function get_all_lelang($year, $offset, $limit)
	{
		...
		// select
		$sql = "SELECT doc_id, doc_title, doc_summary, doc_created_date, doc_file
				FROM {$this->doc}
				WHERE doc_channel_id = 25 and doc_year = $year
				ORDER BY doc_id DESC
				LIMIT {$offset}, {$limit} ";
	...
	function get_all_laporan_tahunan($year, $offset, $limit)
	{
		...
		$sql = "SELECT doc_id, doc_title, doc_summary, doc_created_date, doc_file
				FROM {$this->doc}
				WHERE doc_channel_id = 16 and doc_year = $year
				ORDER BY doc_id DESC
				LIMIT {$offset}, {$limit} ";
	...
	function get_all_sbdk($year, $offset, $limit)
	{
		...
		$sql = "SELECT doc_id, doc_title, doc_summary, doc_created_date, doc_file
				FROM {$this->doc}
				WHERE doc_channel_id = 23 and doc_year = $year
				ORDER BY doc_id DESC
				LIMIT {$offset}, {$limit} ";
	...
	function get_all_gcg($year, $offset, $limit)
	{
		...
		$sql = "SELECT doc_id, doc_title, doc_summary, doc_created_date, doc_file
				FROM {$this->doc}
				WHERE doc_channel_id = 21 and doc_year = $year
				ORDER BY doc_id DESC
				LIMIT {$offset}, {$limit} ";
	...
	{
		$get = $this->db->query("SELECT distinct doc_year FROM documents WHERE doc_channel_id = $id");
		return $get;
	}
