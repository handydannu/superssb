...
class Artikel extends CI_Controller {
	...
	function index()
	{
		...
		$mainData['mainContent']  = $this->load->view('content/varticle', $output,true);
	...
	function add($idkanal='')
	{
		...
		$mainData['mainContent']  = $this->load->view('content/varticle_form', $output,true);
	...
	function submit_update() 
	{
	  if($_SERVER['REQUEST_METHOD']=='POST'){
		...
		if ($update){
			...
			if ($_FILES['userfile']['name'] != '') {
				$image_old	= $this->config->item('posts_images_dir').$year.'/'.$month.'/'.$day.'/'.$ID.'/'.$EDIT['c_images_content'];
