...
class Video extends CI_Controller {
	...
	function index()
	{
		...
		$mainData['bottom_js'] .= add_js('js/data/video.js');

		$mainData['mainContent']  = $this->load->view('video/vlist', $output,true);
	...
	function json()
	{
		$article = $this->contents_model->get_video_datatable();
		print_r($article);
	}
	...
	function add($idkanal='')
	{
		...
		$mainData['mainContent']  = $this->load->view('video/vform', $output,true);
	...
	function submit_update() 
	{
	  if($_SERVER['REQUEST_METHOD']=='POST'){
		...
		if ($update){
			...
			$this->session->set_userdata('message','Video has been updated.');
			$this->session->set_userdata('message_type','success');
		}else{
			$this->session->set_userdata('message','Error Saving Data.');
			$this->session->set_userdata('message_type','error');
