...
class Events extends CI_Controller {
	...
	function index()
	{
		...
		$mainData['bottom_js'] .= add_js('js/data/events.js');

		$mainData['mainContent']  = $this->load->view('informasi/vevents', $output,true);
		$this->load->view('vbase',$mainData);
	}

	function json()
	{
		$data = $this->events_model->get_events();
		print_r($data);
	}
	...
	function add()
	{
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$mainData['mainContent'] = $this->load->view('informasi/vevents_form', $output, TRUE);
			...
			$data['ev_title']            = $this->security->sanitize_filename($post['title']);
			$data['ev_startdate']        = $post['startdate'];
			$data['ev_enddate']          = $post['enddate'];
			$data['ev_starttime']        = $post['starttime'];
			$data['ev_endtime']          = $post['endtime'];
			$data['ev_address']          = in($post['address']);
			$data['ev_summary']          = in($post['summary']);
			$data['ev_description']      = in($post['description']);
			$data['ev_images_thumbnail'] = $file_image_thumb;
			$data['ev_images_content']   = $file_image_name;
			$data['ev_images_caption']   = $this->security->sanitize_filename($post['img_caption']);
			$data['ev_location']         = $this->security->sanitize_filename($post['location']);
			$data['ev_status']           = $post['status'];
			$data['ev_create_date']      = date("Y-m-d H:i:s");

			// _d($data);
			// exit;

			$insertID = $this->events_model->insert_new_event($data);
			if ($insertID){
				if($_FILES['userfile']['name'] != ''){
					// ------- Upload Image file --------
					$destination = $this->config->item('events_images_dir'). date("Y").'/'. date("m").'/'. date("d").'/'. $insertID."/";
	...
	function edit()
	{
		...
		$output['PAGE_TITLE'] = 'EDIT EVENT';
		$output['add_mode']   = 2; // sbg tanda edit
		$output['EDIT']       = $this->events_model->get_event_byid($ID);
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			// ----- Process Image Name. If it's Changed -----
			if ($_FILES['userfile']['name'] != '') {
				$valid_chars_regex = 'A-Za-z0-9_-\s '; // Characters allowed in the file name (in a Regular Expression format)
				$img_info          = pathinfo($_FILES['userfile']["name"]);
				$fileName          = preg_replace('/[^'.$valid_chars_regex.']|\.+$/i', '', strtolower(str_replace(' ', '-', $img_info['filename'])));
				...
				// ---- No change, use from Database
				$file_image_name   = $EDIT['ev_images_content'];
				$file_image_thumb  = $EDIT['ev_images_thumbnail'];
			}
			// ----- END Process Image Name -----
			...
			$data['ev_location']         = $this->security->sanitize_filename($post['location']);
			$data['ev_status']           = $post['status'];
			$data['ev_create_date']      = $post['ev_created'];

			$update = $this->events_model->update_event($data, $ID);
			if ($update){
				if ($_FILES['userfile']['name'] != '') {
					// Delete old image. Upload the new one.
					$tgl   = explode(' ', $EDIT['ev_create_date']);	// [0] =2014-09-22 [1] =10:00:00
                  	$tgl   = explode('-', $tgl[0]);
					$year  = $tgl[0];
					$month = $tgl[1];
					$day   = $tgl[2];
					...
					// Delete old Image
					unlink($this->config->item('events_images_dir').$year.'/'.$month.'/'.$day.'/'.$ID.'/'.$EDIT['ev_images_content']);
					unlink($this->config->item('events_images_dir').$year.'/'.$month.'/'.$day.'/'.$ID.'/'.$EDIT['ev_images_thumbnail']);
					
					// Upload New Image
					$destination = $this->config->item('events_images_dir'). $year.'/'.$month.'/'.$day.'/'.$ID.'/';
					if (!is_file($destination.$file_image_name)) {
						mkdir_r($destination);
					}
	...
	function delete()
	{
		// error_reporting(E_ALL);
		$ID     = $this->uri->segment(3);
		$data   = $this->events_model->get_event_byid($ID);
		$remove = $this->events_model->delete_event($ID);
		
		$tgl   = explode(' ', $data['ev_create_date']);
		$tgl   = explode('-', $tgl[0]);
		$year  = $tgl[0];
		$month = $tgl[1];
		$day   = $tgl[2];
		...
		if($remove){
			if (!empty($data['ev_images_content'])) {
				// Delete image
				unlink($this->config->item('events_images_dir').$year.'/'.$month.'/'.$day.'/'.$ID.'/'.$data['ev_images_thumbnail']);
				unlink($this->config->item('events_images_dir').$year.'/'.$month.'/'.$day.'/'.$ID.'/'.$data['ev_images_content']);
				// Delete the image's folder
				rmdir($this->config->item('events_images_dir').$year.'/'.$month.'/'.$day.'/'.$ID);
			}
			$this->session->set_userdata('message','Event berhasil dihapus.');
			$this->session->set_userdata('message_type','success');
		}else{
