...
class Mobil_kas extends CI_Controller {
	...
	function index()
	{
		...
		$output['PAGE_TITLE'] = 'Mobil Kas Keliling';
		...
		$mainData['bottom_js'] .= add_js('js/data/mobil_kas.js');

		$mainData['mainContent']  = $this->load->view('jaringan/vmobil_kas_form', $output,true);
	...
	function add()
	{
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$mainData['mainContent'] = $this->load->view('jaringan/vmobil_kas_add_form', $output, TRUE);
			...
			$data['mk_name']        = in($post['mk_name']);
			$data['mk_address']     = in($post['mk_address']);
			$data['mk_status'] 		= in($post['mk_status']);
			$data['mk_created']  	= in($post['mk_created']);
			$data['mk_modified']  	= date("Y-m-d H:i:s");

			$insertID = $this->jaringan_model->insert_new_page('mobil_kas', $data);
			if ($insertID){
	...
	function edit()
	{
		...
		$output['EDIT']       = $this->jaringan_model->get_mobil_kas_byid($ID);
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$update = $this->jaringan_model->update_mobil_kas($data, $ID);
			if ($update){
	...
	function delete()
	{
		// error_reporting(E_ALL);
		$ID = $this->uri->segment(3);
		$data         = $this->jaringan_model->get_mobil_kas_byid($ID);
		$remove_quote = $this->jaringan_model->delete_jaringan('mobil_kas', 'mk_id', $ID);
		
		$tgl   = explode(' ', $data['mk_created']);
		$tgl   = explode('-', $tgl[0]);
		$year  = $tgl[0];
		$month = $tgl[1];
		$day   = $tgl[2];
	...
	{
		// error_reporting(E_ALL);
		$this->form_validation->set_rules('mk_name', 'Nama', 'trim|required');
		$this->form_validation->set_rules('mk_address', 'Alamat', 'trim|required');
