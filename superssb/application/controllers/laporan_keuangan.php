...
class Laporan_keuangan extends CI_Controller {
	...
	function index()
	{
		$output['PAGE_TITLE'] = strtoupper(str_replace(array('-', '_'), ' ', $this->uri->segment(1)));
		...
		$mainData['bottom_js'] .= add_js('js/data/laporan_keuangan.js');

		$mainData['mainContent']  = $this->load->view('download/vlaporan_keuangan', $output,true);
		$this->load->view('vbase',$mainData);
	}

	function json()
	{
		$data = $this->download_model->get_laporan_keuangan();
		print_r($data);
	}
	...
	{		
		...
		$replace = array('-', '_');
		$output['PAGE_TITLE'] = 'ADD '.strtoupper(str_replace($replace, ' ', $this->uri->segment(1)));
		...
		if ($this->form_validation->run() == FALSE)
		{
			...
			$output['channels'] = $this->download_model->get_channel_laporan_keuangan();
			
			$mainData['mainContent'] = $this->load->view('download/vlaporan_keuangan_form', $output, TRUE);
	...
	function edit()
	{
		...
		$output['PAGE_TITLE'] = 'EDIT ' .strtoupper(str_replace(array('-', '_'), ' ', $this->uri->segment(1)));
