...
class Report_model extends CI_Model
{
	...
	function article_popular_today($limit)
	{
		...
		$sql = "SELECT 
			...
			LEFT JOIN {$this->features} f ON(f.fe_id=c.c_feature)
			LEFT JOIN {$this->user} u ON(u.uid=c.c_author)
			LEFT JOIN {$this->user} e ON(e.uid=c.c_editor)
	...
	{
		$sql = "SELECT * FROM {$this->contact} ORDER BY contact_id DESC LIMIT {$limit} ";
		$query = $this->db->query($sql);
		$ret = $query->result_array();
		$query->free_result();
		return $ret;
